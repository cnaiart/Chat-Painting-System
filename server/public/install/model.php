...
class installModel
{
    ...
    /**
     * @notes gd库，png及jpeg校验
     * @return string
     * @author mjf
     * @date 2023/11/15 14:27
     */
    public function checkGdPngAndJpeg()
    {
        $gd = $this->checkGd2();
        if ($gd == 'ok') {
            $gdInfo = gd_info();
            if (isset($gdInfo['PNG Support']) && $gdInfo['PNG Support'] === true
                && isset($gdInfo['JPEG Support']) && $gdInfo['JPEG Support'] === true) {
                return 'ok';
            }
        }
        return 'fail';
     ...
     */
    public function checkSwooleLoader()
    {
        return $result = extension_loaded('swoole_loader') ? 'ok' : 'fail';
    }
     ...
     */
    public function checkSwooleLoaderRemark()
    {
        $remark = $this->swooleLoaderIsThreadSafety() ? '线程安全扩展' : '非线程安全扩展';
        return $remark;
...
//            }
            ...
            //替换表前缀，只替换一次
            $replace = $this->name.'.`'.$this->prefix;
            $table = preg_replace('/`ai_/', $replace, $table,1);
...
//        exit();
    ...
    public function importDemoData()
    {
        ...
        foreach ($insertTables as $table) {
            ...
            $table = str_replace('`ai_', $this->name . '.`ai_', $table);
            $table = str_replace('`ai_', '`' .$this->prefix, $table);
            if ( !$this->dbh->query($table)) return false;
        }
    ...
    public function initAccount($post)
    {
        ...
        // 超级管理员
        $sql = "INSERT INTO `ai_admin`(`id`, `root`, `name`, `avatar`, `account`, `password`, `login_time`, `login_ip`, `multipoint_login`, `disable`, `create_time`, `update_time`, `delete_time`) VALUES (1, 1, '{$post['admin_user']}', '', '{$post['admin_user']}', '{$password}','{$time}', '', 1, 0, '{$time}', '{$time}', NULL);";
        // 超级管理员关联部门
        $sql .= "INSERT INTO `ai_admin_dept` (`admin_id`, `dept_id`) VALUES (1, 1);";
     ...
     */
    function swooleLoaderIsThreadSafety()
    {
        ob_start();
        phpinfo();
