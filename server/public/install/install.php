...
$post = [
    ...
    'name' => $_POST['name'] ?? 'chat',
    'admin_user' => $_POST['admin_user'] ?? '',
    'admin_password' => $_POST['admin_password'] ?? '',
    'admin_confirm_password' => $_POST['admin_confirm_password'] ?? '',
    'prefix' => $_POST['prefix'] ?? 'ai_',
...
if ($step == 4) {
    ...
    if (empty($post['user'])) {
        ...
        $canNext = false;
        $message = '管理员密码不能为空';
    } elseif (strlen(trim($post['admin_password'])) < 5 || strlen(trim($post['admin_password'])) > 32){
        $canNext = false;
        $message = '管理员密码长度在6到32之间';
