...
//                $member_end_time = $user['member_end_time'] - $deduct_time;
                ...
                switch ($duration_type) {
                    case MemberPackageEnum::DURATION_TYPE_MONTH:
                        $member_package_time = strtotime("+{$member_package['duration']} month",$order['pay_time']);
                        break;
                    case MemberPackageEnum::DURATION_TYPE_DAY:
                        $member_package_time = strtotime("+{$member_package['duration']} day",$order['pay_time']);
                        break;
                }

                $add_time = $member_package_time - $order['pay_time'];
                $time = time();
                $member_end_time = $user['member_end_time'];
                if ($user['member_end_time'] > $time) {
                    $member_end_time = $user['member_end_time'] - $add_time;
                    if ($member_end_time < $time) {
            ...
            if ($give_chat_number > 0) {
                User::update(['balance'=>['dec',$give_chat_number]],['id'=>$user['id']]);
                AccountLogLogic::add(
                    $user['id'],
