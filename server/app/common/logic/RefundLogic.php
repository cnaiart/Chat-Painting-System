...
class RefundLogic extends BaseLogic
{
    ...
    public static function refund($order, $refundRecordId, $refundAmount, $handleId)
    {
        ...
        try {
            switch ($order['pay_way']) {
                ...
                self::EPayRefund($order, $refundAmount,$refundRecordId);
                    break;
                default:
     ...
     * @param $refundRecordId
     * @author ljj
     * @date 2024/1/15 5:41 下午
     */
    public static function EPayRefund($order, $refundAmount,$refundRecordId)
    {
        $result = (new EpayService())->refund($order, $refundAmount);
        $result = (array)$result;

        if ($result['code'] === 0) {
            $refundStatus = RefundEnum::REFUND_SUCCESS;
        } else {
            $refundStatus = RefundEnum::REFUND_ERROR;
        }
        //更新日志
        RefundLog::update([
